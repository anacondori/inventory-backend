...
public class CategoryServiceImpl implements ICategoryService{
	...
	public ResponseEntity<CategoryResponseRest> save(Category category) {
		...
		try {
			Category categoriSaved = categoryDao.save(category);
			if (categoriSaved != null) {
				list.add(categoriSaved);
				response.getCategoryResponse().setCategory(list);
				response.setMetadata("Respuesta OK", "00", "Categoria insertada");
			} else {
				response.setMetadata("Respuesta KO", "-1", "Categoria no insertada");
	...
	public ResponseEntity<CategoryResponseRest> update(Category category, Long id) {
		...
		try {
			Optional<Category> categoryId = categoryDao.findById(id);
			if (categoryId.isPresent()) { 
				...
				Category categoryToUpdate = categoryDao.save(categoryId.get()); //con esta sentencia se actualiza a la BBDD, la respuesta se asigna a la variable
			...
			response.setMetadata("Respuesta KO", "-1", "Error al actualizar la categoria");
			e.getStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	public ResponseEntity<CategoryResponseRest> deleteById(Long id) {
		...
		try {
			...
			response.setMetadata("Respuesta KO", "-1", "Error al eliminar la categoria");
			e.getStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
